...
print(summary(fit.hla.bin))


cat("fitted values for hlabin, hla-gaussian, hla-gaussian-hi-variance\n")

print(fitted(fit.hla.bin)[1:20])

print(fitted(fit.resp.hla)[1:20])
print(fitted(fit.respvar.hla)[1:20])

cat("vcov for hlabin, hla-gaussian, hla-gaussian-hi-variance\n")

print(vcov(fit.hla.bin)[1:20,1:20])

print(vcov(fit.resp.hla))
print(vcov(fit.respvar.hla))

cat("residuals  for hlabin, hla-gaussian, hla-gaussian-hi-variance\n")

print(residuals(fit.hla.bin, type="deviance")[1:20])
print(residuals(fit.hla.bin, type="pearson")[1:20])
print(residuals(fit.hla.bin, type="response")[1:20])

print(residuals(fit.resp.hla)[1:20])
print(residuals(fit.respvar.hla)[1:20])
